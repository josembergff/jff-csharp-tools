...
namespace JffCsharpTools9.Apresentation.Controllers
{
    ...
    public abstract class DefaultGuidCRUDController<TService, TContext, TEntity> : DefaultController
        where TService : IDefaultGuidService<TContext>
        ...
        where TEntity : DefaultGuidEntity<TEntity>, new()
    {
        ...
        public DefaultGuidCRUDController(TService serviceCrud, ILogger<DefaultController> logger, bool filterCurrentUser = true) : base(logger)
        {
        ...
        public virtual async Task<ActionResult<IEnumerable<TEntity>>> Get()
        {
            ...
            {
                var returnObj = await serviceCrud.GetByUser<TEntity>(CurrentGuidUser_FromBearerToken);
        ...
        public virtual async Task<ActionResult<IEnumerable<TEntity>>> GetFilter([FromQuery] TEntity filter)
        {
            if (filterCurrentUser)
            {
                var returnObj = await serviceCrud.GetByUser<TEntity>(CurrentGuidUser_FromBearerToken, entityFilter: filter);
                return ReturnAction(returnObj);
            }
            else
            {
        ...
        {
            var returnObj = await serviceCrud.GetPaginated(filter, f => true, IdUser: CurrentGuidUser_FromBearerToken, filterCurrentUser: filterCurrentUser);
            return ReturnAction(returnObj);
        }
        ...
        {
            var returnObj = await serviceCrud.GetByKey<TEntity, int>(CurrentGuidUser_FromBearerToken, key, filterCurrentUser: filterCurrentUser);
            return ReturnAction(returnObj);
        }
        ...
        {
            var returnObj = await serviceCrud.Create(CurrentGuidUser_FromBearerToken, value, filterCurrentUser: filterCurrentUser);
            return ReturnAction(returnObj);
        }
        ...
        {
            var returnObj = await serviceCrud.UpdateByKey(CurrentGuidUser_FromBearerToken, value, key, filterCurrentUser: filterCurrentUser);
            return ReturnAction(returnObj);
        }
        ...
        {
            var returnObj = await serviceCrud.DeleteByKey<TEntity, int>(CurrentGuidUser_FromBearerToken, key, filterCurrentUser: filterCurrentUser);
            return ReturnAction(returnObj);
        }
    }
}